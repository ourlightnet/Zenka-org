...
export const projects: Project[] = [
  ...
  },
  {
    ...
    longDescription:
      "The premise: beings of every kind, including ancestors, angels, extraterrestrials, ascended masters, and our own higher selves, are watching us and trying to communicate, while we barely notice. The 48 Hour Comedy Lab lives in the gap between what they are saying and what we are (just barely) hearing, with cameos from the ET races of the galactic age as they make their first appearances. Over one immersive weekend in Sedona, comedians, editors, chefs, and production artists write and shoot a slate of clean, uplifting short-form clips on this theme, then open the doors for a public release party at the Humankind Center. Led by Zenka Caro and the LightNet team with local performers Daniel Flynn (Mary D. Fisher theater, Super Improv) and Jesselynn Desmond (Sedona improv), the lab pairs an in-person community event with a YouTube, Instagram, and TikTok campaign that keeps Sedona on the cultural map long after the weekend ends.",
